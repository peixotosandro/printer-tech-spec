...
def find_equipments(input_text):
    ...
    prompt = f"""
Você é um assistente de IA altamente inteligente especializado em encontrar dispositivos de fabricantes que correspondam a especificações técnicas fornecidas pelo usuário. Use as especificações mais recentes e precisas dos fabricantes (ex.: Lexmark, Xerox, HP, Ricoh, Epson, Brother, ou outros sites oficiais relevantes) até a data atual {current_date}. O usuário fornecerá um texto contendo os fabricantes desejados (ex.: Lexmark, HP, Canon) e as especificações técnicas (ex.: impressora, multifuncional, 40 ppm, tela 4,3 polegadas).
